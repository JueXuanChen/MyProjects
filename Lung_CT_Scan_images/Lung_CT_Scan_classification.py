...
# 匯入資料，並將資料建立成Dataframe
# Download latest version
path = kagglehub.dataset_download("orvile/ct-scan-images")

print("Path to dataset files:", path)

path = path + "/CT-Scan images/CT-Scan images/CT-Scan Images"
...
# 對資料進行augamentation and normalization
...
learning_rate_reduction = ReduceLROnPlateau(monitor='val_accuracy', patience=1, verbose=1, factor=0.2, min_lr=0.00000001)
...
model.save('Lung_CT_scan_model.keras')
model.save('Lung_CT_scan_model.h5')

# 繪製trainig/validaiotn的acc/loss，評估模型訓練的狀況
...
plt.savefig('Lung_CT_scan_image_Acc_loss.png')
plt.show()

# 利用test set對訓練好的模型進行預測，並評估模型的預測效能
...
plt.savefig('Lung_CT_scan_image_CM.png')
plt.show()
