...
path = path + "/Brain_Cancer raw MRI data/Brain_Cancer"
...
# 對資料labeling，並計算各標籤比例權重
label_mapping = {'brain_glioma': '0', 'brain_menin': '1', 'brain_tumor': '2'}
...
# 對資料進行augementation and normalization
...
# 匯入Inception_v3模型，並建立要訓練的深度學習模型
...
# 設定模型訓練條件與策略，接著利用training set and validation set對模型進行訓練，再來儲存訓練好的模型
...
learning_rate_reduction = ReduceLROnPlateau(monitor='val_accuracy', patience=3, verbose=1, factor=0.3, min_lr=0.00000001)
...
# 繪製train/validation的acc/loss，來評斷模型訓練狀況
...
plt.savefig('Brain_MRI_model_Acc_loss.png')
plt.show()
...
plt.savefig('Brain_MRI_model_CM.png')
plt.show()
